 ...
 */
    ...
    public void PostNewSpartan3() {
        //Optional homeworks
        //Homework-1
        //1-Create csv file from mockaroo website, which includes name,gender,phone
        //2-Download csv file
    ...
    @Test
    public  void hw() {
        Response response=when().get("https://my.api.mockaroo.com/eu3gul?key=f0303ca0");

        List<Map<String, Object>> allSpartanList = response.body().as(List.class);

        for(int i=0;i<allSpartanList.size();i++) {
            Map<String, Object> requestMap = new HashMap<>();

            requestMap.put("name", allSpartanList.get(i).get("name"));
            requestMap.put("gender", allSpartanList.get(i).get("gender"));
            requestMap.put("phone", new BigDecimal(String.valueOf(allSpartanList.get(i).get("phone"))));
