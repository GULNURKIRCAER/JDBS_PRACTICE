...
public class hrApiWithPath {
    ...
    public void getCountriesWithPath(){
        ...
        //String link2 =response.path("items.links[2].rel[0]");
        String link2 =response.path("items.links[2].href[0]");// 0 koymazsak LIST Olur
        System.out.println("link2 = " + link2);

        //get all countries
        List<String> countryNames = response.path("items.country_name");// index num olmazsa direkt list oluyor
        System.out.println("countryNames = " + countryNames);

        //assert that all regions_id's are equal to 2
        List<Integer> regionIDs = response.path("items.region_id");
        for (Integer regionID : regionIDs) {
            //System.out.println(regionID);
            assertTrue(regionID.equals(2));

//        for (int regionID : regionIDs) { BU DA OLUR
//             System.out.println(regionID);
